...
//                matrixCheck = matrixCheck();  
        ...
        case MotionEvent.ACTION_POINTER_UP:  
        	if (mode == ZOOM && matrixZoomCheck) {  
	            ...
	            } else if (x3 > 0 && y3 < BaseValue.screenWidth) {
	            	matrixZoom.postTranslate(0 - x3, BaseValue.screenWidth - y3);// 平移
	            } else if (x1 > 0 && y1 < 0 && y3 > BaseValue.screenWidth) {
	            	matrixZoom.postTranslate(0 - x1, 0);// 平移   
	            } else if (x1 < 0 && y1 > 0 && x2 > BaseValue.screenWidth) {
	            	matrixZoom.postTranslate(0, 0 - y1);// 平移   
	            } 
	            else if (x2 < BaseValue.screenWidth && y2 > 0) {
	            	matrixZoom.postTranslate(BaseValue.screenWidth - x2, 0 - y2);// 平移
	            } else if (x2 < BaseValue.screenWidth && y2 < 0 && y4 > BaseValue.screenWidth) {
	            	matrixZoom.postTranslate(BaseValue.screenWidth - x2, 0);// 平移
	            } 
	            else if (x4 < BaseValue.screenWidth && y4 < BaseValue.screenWidth) {
	            	matrixZoom.postTranslate(BaseValue.screenWidth - x4, BaseValue.screenWidth - y4);// 平移
	            } else if (x4 > BaseValue.screenWidth && y4 < BaseValue.screenWidth && x3 < BaseValue.screenWidth) {
	            	matrixZoom.postTranslate(0, BaseValue.screenWidth - y4);// 平移
	            }
	            matrix.set(matrixZoom);  
	            invalidate();  
        	}
